...
class ModeManager:
    ...
    def __init__(self, max_speed=9.0, follow_cap=1.0, overtake_cap=2.0,
        ...
        self._speed_map = {
            ...
            DrivingMode.STATIC_AVOID: max_speed * 0.7,
...
class CycleRecord:
    ...
    timestamp: float = 0.0; lap_id: int = 0
    mode: int = 0; mode_name: str = ""
    steer: float = 0.0; speed: float = 0.0; actual_speed: float = 0.0
    mpc_ok: bool = False; mpc_ms: float = 0.0
    blend_weight: float = 0.0; front_clearance: float = 0.0
    pos_x: float = 0.0; pos_y: float = 0.0; heading: float = 0.0
...
class ExperimentLogger:
    ...
    def __init__(self, output_dir="~/experiment_data"):
        ...
        self._lap = 0; self._t0 = time.time()
    ...
    def save_all(self):
        if self._records: self.finish_lap()
...
class BattleVehicleNode(Node):
    ...
    def build_local_ref(self, front_state, target_speed, speed_upper_bound, current_pose=None):
        ...
        if self.raceline_array is None or current_pose is None:
            ...
            fused_heading = self._bias_heading_with_laser(fused_heading, front_state, weight_global=weight_global)
            blend_steps = min(4, self.mpc_horizon)
    ...
    def pure_pursuit_speed(self, ref, base_speed):
        ...
        speed_factor = 1.0 / (1.0 + 0.8 * abs(curvature) * self.wheelbase)
...
def main(args=None):
    ...
    try:
        ...
        battle_node.get_logger().info('正在保存实验数据...')
        battle_node.exp_logger.save_all()
        battle_node.get_logger().info('实验数据已保存')
    finally:
