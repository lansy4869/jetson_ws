...
class BattleVehicleNode(Node):
    def __init__(self):
        ...
        if self.predictive_shield is not None:
            self.get_logger().info(
                ...
                % (
                    self.predictive_shield.shadow_mode,
                    self.predictive_shield.speed_only,
                    self.predictive_shield.adaptive_margin_enabled,
                )
            )
        else:
            self.get_logger().warn("AC-ARPS shield disabled (shield_enabled=false).")
    ...
    # === 【AC-ARPS 位置三】发布前的安全过滤（分级介入 + 调试话题）===
    def apply_predictive_shield(self, data, raw_ranges, drive_msg):
        """对 drive_msg 中的基础指令做 AC-ARPS 过滤，返回过滤后的 drive_msg。
        ...
        base_speed = float(drive_msg.drive.speed)
        base_steering = float(drive_msg.drive.steering_angle)
        try:
            safe_speed, safe_steering, shield_debug = self.predictive_shield.filter(
                ranges=raw_ranges,
                ...
                stamp_sec=(
                    float(data.header.stamp.sec)
                    + float(data.header.stamp.nanosec) * 1e-9
                ),
            )
        except Exception as exc:
            self.get_logger().error(
                f"AC-ARPS shield failed, passing base command through: {exc}")
            return drive_msg

        drive_msg.drive.speed = float(safe_speed)
        drive_msg.drive.steering_angle = float(safe_steering)

        self.shield_debug_pub.publish(make_shield_debug_msg(shield_debug))
        if shield_debug["intervention_level"] > 0:
            self.get_logger().info(
                "AC-ARPS L%d(%s)%s v %.2f->%.2f δ %.3f->%.3f "
                "clr=%.2f ttc=%.2f q=%.3f %.1fms"
                % (
                    shield_debug["intervention_level"],
                    shield_debug["intervention_reason"],
                    "" if shield_debug["applied"] else "[shadow]",
                    shield_debug["base_speed"],
                    shield_debug["suggested_speed"],
                    shield_debug["base_steering"],
                    shield_debug["suggested_steering"],
                    shield_debug["predicted_clearance"],
                    min(shield_debug["ttc"], 99.0),
                    shield_debug["conformal_margin"],
                    shield_debug["compute_time_ms"],
                )
            )
        return drive_msg
    ...
    def middle_line_callback(self, data):
        ...
        # 【AC-ARPS 位置三】Follow/chaoche 已作用到基础速度之后、发布之前过滤：
        # 安全时完全透传；不安全时先只减速(保持δ)，减速仍不安全才加小幅转角残差
